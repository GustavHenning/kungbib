...
Utmaningen i att segmentera nyhetsartiklar återfinns i den otroliga mångfald av utseende och format. Innehållet är även detta  mångfaldigt, där man återfinner allt ifrån faktamässiga artiklar, till debatter, listor av fakta och upplysningar, reklam och väder bland annat. Otroliga framsteg har gjorts inom djupinlärning just för objektdetektering och semantisk segmentering bara de senaste årtiondet. Frågan vi ställer oss är: \textit{Kan samma metodik appliceras inom domänen nyhetsartiklar?}
...
The dataset created in this project consists of published newspapers which have seen wide circulation.
No special consideration needs to be taken in terms of sensitive or personally identifiable information.
...
If two elements discuss the same topic but have different authors they will be considered two different news units.

Multi page articles are content pages in which the content spans multiple pages.
Thus, a single page may only display half of an image, or a few elements of the complete body text.
If the page is the first of a multi page article the byline may be missing, but the elements will still be considered a news unit.
Similarly, if the page is not the first of a multi page article, a headline may be missing, but the elements will still count as a news unit.
...
A broad overview of the different datasets labeled specifically for this thesis will be introduced.
Then a formal definition of a news unit will be made, which will be derived from the definition of a news article.
This definition will be based on what a user might like to query as content on an article-level search of a newspaper.
...
A \textit{news unit} may contain all of the above, including an image of the author themselves and smaller elements of text that usually contain explanatory contents, such as definitions or facts associated with the article itself.
...
which are then included by extending the border of the original rectangular shape in an asymmetric manner.

\begin{figure}[H]
\begin{tikzpicture}
\draw[step=1cm,white,very thin] (-2.9,-2.9) grid (6.9,6.9);
\draw[red] (-2,-2) -- (2,-2) -- (2,2) -- (6,2) -- (6,6) -- (-2,6) -- (-2,-2);

\node[draw] at (0,2) {A};

\draw[red, densely dotted] (2,2) -- (2,6);
...
Example where article cannot be labeled as one unit.
...
In this project, these types of advertisements are labeled as news articles.
...
In this thesis we are concerned with the fact that the elements are labeled as the same unit by the model.
The classes are made in an attempt to support the model's ability to discriminate between units of content.

\subsubsection{Listing}

Dense lists of information has received its own class. As seen in \textbf{reference to visual description of listings},
...
\textbf{TODO describe what is found in mstrain files img norm cfg, preprocessing steps, optimizer, lr config}
\textbf{TODO describe steps to use text-features}
\textbf{TODO describe steps to use 1 class}
\textbf{TODO maybe in the next section}
...
In this section we will introduce the experiments that will be conducted in order to attempt to achieve the goals outlined in \autoref{}.
...
As mentioned in the background \textbf{TODO}, Mask R-CNN performs worse on smaller objects. An experiment will be conducted to investigate if we can exploit the large image sizes in our dataset.
