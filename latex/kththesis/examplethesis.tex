...
%% This template has been adapted by Anders Sj\"ogren to the University
...
% \todo[inline, backgroundcolor=kth-lightblue]{Text p\aa  svenska}
...
\alttitle{Artikelsegmentering med multimodala artificiella neuronn\"atverk}
\altsubtitle{Med hj\"alp av Mask R-CNN och sentence transformers}
...
\presentationRoom{via Zoom https://kth-se.zoom.us/j/64146298953}
\presentationAddress{Drottning Kristinas v\"ag 4, 114 28 Stockholm}
\presentationCity{Stockholm}

% When there are multiple opponents, separate their names with '\&'
% Opponent's information
\opponentsNames{Nadir Ait Lahmouch}
...
% 10202 Systemvetenskap, informationssystem och informatik (samh\"allsvetenskaplig inriktning under 50804)
% Information Systems (Social aspects to be 50804)
% 10203 Bioinformatik (ber\"akningsbiologi) (till\"ampningar under 10610)
% Bioinformatics (Computational Biology) (applications to be 10610)
% 10204 M\"anniska-datorinteraktion (interaktionsdesign) (Samh\"allsvetenskapliga aspekter under 50803) Human Computer Interaction (Social aspects to be 50803)
...
% 10208 Spr\aa kteknologi (spr\aa kvetenskaplig databehandling) Language Technology (Computational Linguistics)
...
% 20207 Inb\"addad systemteknik Embedded Systems
...
\SwedishKeywords{Historiska tidningar, Bildsegmentering, Multimodal inl\"arning, Djupinl\"arning, Digital humaniora, Mask R-CNN}
...
% Abstract fungerar p\aa  svenska ocks\aa .
...
\begin{abstract}
    \markboth{\abstractname}{}
\begin{scontents}[store-env=lang]
Historiska tidningar, Bildsegmentering, Multimodal inl\"arning, Djupinl\"arning, Digital humaniora, Mask R-CNN
\end{scontents}
\begin{scontents}[store-env=abstracts,print-env=true]

I detta och det f\"orra \aa rhundradet har kraftiga \aa taganden gjorts f\"or att digitalisera traditionellt medieinneh\aa ll som tidigare endast tryckts i pappersformat. F\"or att kunna st\"odja s\"okningar och fasetter i detta inneh\aa ll kr\"avs bearbetning p\aa  semantisk niv\aa , det vill s\"aga att inneh\aa llet styckas upp p\aa  artikelniv\aa , ist\"allet f\"or per sida. Trots att m\"anniskor har l\"att att dela upp inneh\aa ll p\aa  semantisk niv\aa , \"aven p\aa  ett fr\"ammande spr\aa k, forts\"atter arbetet f\"or automatisering av denna uppgift.

Utmaningen i att segmentera nyhetsartiklar \aa terfinns i m\aa ngfalden av utseende och format. Inneh\aa llet \"ar \"aven detta  m\aa ngfaldigt, d\"ar man \aa terfinner allt ifr\aa n faktam\"assiga artiklar, till debatter, listor av fakta och upplysningar, reklam och v\"ader bland annat. Stora framsteg har gjorts inom djupinl\"arning just f\"or objektdetektering och semantisk segmentering bara de senaste \aa rtiondet. Fr\aa gan vi st\"aller oss \"ar: \textit{Kan samma metodik appliceras inom dom\"anen nyhetsartiklar?}

Dessa modeller \"ar skapta f\"or att klassificera v\"arldsliga ting. I denna dom\"an har vi tillg\aa ng till texten och dess koordinater via en potentiellt bristf\"allig optisk teckenigenk\"anning. Vi unders\"oker ett s\"att att utnyttja denna textinformation i ett f\"ors\"ok att f\"orb\"attra resultatet i denna specifika dom\"an. Baserat p\aa  data fr\aa n Kungliga Biblioteket unders\"oker vi hur v\"al denna metod l\"ampar sig f\"or uppstyckandet av inneh\aa ll i tidningar l\"angsmed tidsperioder d\"ar designen f\"or\"andrar sig markant. Resultaten visar att Mask R-CNN l\"ampar sig v\"al f\"or anv\"andning inom dom\"anen nyhetsartikelsegmentering, \textit{\"aven utan texten som input till modellen}.
...
%\todo[inline, backgroundcolor=kth-lightblue]{Nyckelord som beskriver inneh\aa llet i uppsatsrapporten}
\end{scontents}
\end{abstract}
...
The selected methods, based on previous work which can be found in \Cref{sec:relwork} from researchers investigating a multimodal approach to newspaper article segmentation, are then evaluated on the dataset created during this project.
...
In \Cref{ch:background}, we introduce the historical background to understand the subject matter, the data we are about to label, the preliminaries for grasping the proposed solution, and recap the previous work in document segmentation as well as language models.
Finally the metrics used to quantitatively describe the results are detailed.

\Cref{ch:method} details the sampling of data and the labeling strategy of the dataset created from the data accessed at the National Library of Sweden.
Next, the neural network employed in the study is described in terms of implementation in \Cref{sec:modelarch}, and the relevant adaptations to the pre-processing and manipulation of the data is explained.
What follows is a description of the experiments conducted to test our formulated goals in \Cref{sec:experiments}, as well as the tools used to perform them in \Cref{sec:sysdoc}.

The results are presented in \Cref{ch:results} and discussed in \Cref{ch:disc},
...
In this chapter, the history of digitization of historical documents will be reviewed. A description of the data accessed at the National Library of Sweden will be produced. Details about how the deep learning model functions will be introduced in \Cref{sec:preliminaries}. Then, we will look at the history of segmentation algorithms, as well as the history of the deep learning models that we intend to extend and evaluate. 
...
As previously stated in\ \Cref{ch:introduction}, in this thesis we refer to blocks of text or images as \textit{elements} or \textit{components} of content, where \textit{content} is any type of information meant to be conveyed through text or image.
...
which are then included by extending the border of the original rectangular shape in an asymmetric manner as can be seen in \Cref{fig:asymextention}.
...
For example, if a section of a newspaper has very densely formatted text, and the image and image caption is pushed far from its textual contents as seen in ~\Cref{fig:blockingelements}, a decision has to be made for how to label this instance.
...
In this thesis, we consider both the "white" newspaper background and static content, as mentioned in \Cref{subsec:static}, such as logos or footers that appear in the same place for the same typographic design as "Background".
...
For a complete list of definitions of words related to news articles, see ~\Cref{app:newsarticle}
...
In this thesis, these types of advertisements are labeled as "News Units". For more information regarding that decision, see \Cref{subsub:ads}

\subsubsection{Listing}
\label{subsub:labellistings}
Dense lists of information have received its own class. As seen in \Cref{bgsubsec:listings},
...
Movie advertisements sometimes lists available movie information in a list-like format within their advertisement space. Due to reasons of not overlapping labels as seen in \Cref{subsub:overlappinglabels}, such movie advertisements are labeled as "Advertisement". 
...
See \Cref{subsub:listings} for a more thorough discussion.
...
Therefore, if a pixel or a group of pixels are assigned any class (except background) previously mentioned in \Cref{subs:finalclasses}, they will be labeled as a "Publication unit" in the experiments where class confusion is investigated.
A visual example of this procedure can be seen in \Cref{fig:pubunit}.
...
\end{subfigure}}
\end{adjustbox}
  \caption{Ground truth label comparison between labels for 6 classes versus labels using the Publication Unit superclass. Both figures a) and b) depict the labels of the same newspaper image, an image from the DN 2010-2020 Test subset (\Cref{subsub:inset}). }
\label{fig:pubunit}
\end{figure}
...
As described in \Cref{subsec:labelingstrat}, some elements found in the newspaper are simply repeated and do not, by themselves, convey any news content.
...
In this section we will introduce the experiments that will be conducted in order to attempt to achieve the goals outlined in \Cref{sec:goals}.
...
Baseline will be the unaltered Mask R-CNN models as they are implemented in the framework we are using (\Cref{subsub:MMDetection}).
...
The MMDetection~\cite{DBLP:journals/corr/abs-1906-07155} framework was used to conduct the experiments in \Cref{sec:experiments}.
...
Throughout these results, we will divide each section to show results using the classes defined in \Cref{subs:finalclasses} and \Cref{subs:pubunit} respectively.
...
\begin{table}[H]
  \begin{center}
    ...
    \textbf{Final name} & \textbf{Backbone} & \textbf{Sentence transformer} & \textbf{Text embeddings length} \\
    \hline
    r50 & resnet50-FPN & None & 0 \\    \hline
    r101 & resnet101-FPN & None & 0 \\    \hline
    x101-32x4d & resnext101-32x4d-FPN & None & 0 \\    \hline
    x101-32x8d & resnext101-32x8d-FPN & None & 0 \\    \hline
    x101-64x4d & resnext101-64x4d-FPN & None & 0 \\     \hline

    r50-MiniLM & resnet50-FPN & all-MiniLM-L6-v2 & 384 \\     \hline
    r101-MiniLM & resnet101-FPN & all-MiniLM-L6-v2 & 384 \\     \hline
    x101-32x4d-MiniLM & resnext101-32x4d-FPN & all-MiniLM-L6-v2 & 384 \\     \hline
    x101-32x8d-MiniLM & resnext101-32x8d-FPN & all-MiniLM-L6-v2 & 384 \\     \hline
    x101-64x4d-MiniLM & resnext101-64x4d-FPN & all-MiniLM-L6-v2 & 384 \\     \hline

    r50-all-mpnet & resnet50-FPN & all-mpnet-base-v2 & 384 \\     \hline
    r101-all-mpnet & resnet101-FPN & all-mpnet-base-v2 & 384 \\     \hline
    x101-32x4d-all-mpnet & resnext101-32x4d-FPN & all-mpnet-base-v2 & 384 \\     \hline
    x101-32x8d-all-mpnet & resnext101-32x8d-FPN & all-mpnet-base-v2 & 384 \\     \hline
    x101-64x4d-all-mpnet & resnext101-64x4d-FPN & all-mpnet-base-v2 & 384 \\     \hline

    r50-mq-mpnet & resnet50-FPN & multi-qa-mpnet-base-dot-v1 & 384 \\     \hline
    r101-mq-mpnet & resnet101-FPN & multi-qa-mpnet-base-dot-v1 & 384 \\     \hline
    x101-32x4d-mq-mpnet & resnext101-32x4d-FPN & multi-qa-mpnet-base-dot-v1 & 384 \\     \hline
    x101-32x8d-mq-mpnet & resnext101-32x8d-FPN & multi-qa-mpnet-base-dot-v1 & 384 \\     \hline
    x101-64x4d-mq-mpnet & resnext101-64x4d-FPN & multi-qa-mpnet-base-dot-v1 & 384 \\     \hline
...
All model results in this section are sorted by the \textbf{$mAP_m$}, the mean Average Precision for the segmentation mask. 

\begin{table}[H]
  \begin{center}
    \caption{Top 5 results - 6 classes - In-set (\Cref{subsub:inset})}
...
\begin{table}[H]
  \begin{center}
    \caption{Top 5 results - 6 classes - Near-set (\Cref{subsub:nearset})}
    \label{tab:top5allcnearset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l}
    \textbf{Final name} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} \\
    \hline
r50                            & 0.419 & 0.496 & 0.446 & 0.407 & 0.496 & 0.436 \\ \hline
x101-32x4d                     & 0.417 & 0.493 & 0.425 & 0.407 & 0.493 & 0.424 \\ \hline
x101-64x4d                     & 0.416 & 0.509 & 0.414 & 0.406 & 0.491 & 0.412 \\ \hline
x101-32x8d                     & 0.413 & 0.489 & 0.421 & 0.402 & 0.496 & 0.419 \\ \hline
r101                           & 0.408 & 0.470 & 0.433 & 0.405 & 0.468 & 0.430 \\ \hline
...
\begin{table}[H]
  \begin{center}
    \caption{Top 5 results - 6 classes - Out-set (\Cref{subsub:outset})}
    \label{tab:top5allcoutset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l}
    \textbf{Final name} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} \\
    \hline
x101-64x4d-all-mpnet           & 0.510 & 0.597 & 0.547 & 0.497 & 0.598 & 0.545 \\ \hline
r50                            & 0.490 & 0.589 & 0.545 & 0.486 & 0.589 & 0.534 \\ \hline
x101-32x4d                     & 0.484 & 0.587 & 0.520 & 0.474 & 0.585 & 0.515 \\ \hline
x101-64x4d                     & 0.482 & 0.581 & 0.510 & 0.478 & 0.576 & 0.524 \\ \hline
x101-32x8d-MiniLM              & 0.468 & 0.578 & 0.508 & 0.456 & 0.576 & 0.506 \\ \hline
    \end{tabular}}
  \end{center}
\end{table}
...
\begin{table}[H]
  \begin{center}
    \caption{Top 5 results - 1 class - In-set (\Cref{subsub:inset})}
    \label{tab:top5onecinset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l} %
    \textbf{Final name} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} \\
    \hline
r50-1c                         & 0.843 & 0.933 & 0.874 & 0.839 & 0.933 & 0.883 \\ \hline
x101-64x4d-1c                  & 0.843 & 0.916 & 0.871 & 0.839 & 0.916 & 0.871 \\ \hline
x101-32x4d-1c                  & 0.841 & 0.906 & 0.872 & 0.835 & 0.906 & 0.872 \\ \hline
x101-32x8d-1c                  & 0.841 & 0.910 & 0.861 & 0.837 & 0.910 & 0.862 \\ \hline
r101-1c                        & 0.840 & 0.917 & 0.862 & 0.832 & 0.917 & 0.862 \\ \hline
    \end{tabular}}
  \end{center}
\end{table}
...
\begin{table}[H]
  \begin{center}
    \caption{Top 5 results - 1 class - Near-set (\Cref{subsub:nearset})}
    \label{tab:top5onecnearset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l}
    \textbf{Final name} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} \\
    \hline
r50-1c                         & 0.550 & 0.677 & 0.589 & 0.543 & 0.676 & 0.584 \\ \hline
x101-64x4d-1c                  & 0.524 & 0.636 & 0.554 & 0.520 & 0.636 & 0.555 \\ \hline
x101-32x8d-1c                  & 0.522 & 0.634 & 0.551 & 0.519 & 0.636 & 0.552 \\ \hline
r101-1c                        & 0.516 & 0.622 & 0.552 & 0.511 & 0.623 & 0.553 \\ \hline
r50-distilroberta-1c           & 0.508 & 0.636 & 0.548 & 0.498 & 0.636 & 0.534 \\ \hline
    \end{tabular}}
  \end{center}
\end{table}
...
\begin{table}[H]
  \begin{center}
    \caption{Top 5 results - 1 class - Out-set (\Cref{subsub:outset})}
...
r50-distilroberta-1c           & 0.463 & 0.624 & 0.488 & 0.456 & 0.619 & 0.483 \\ \hline
r50-all-mpnet-1c               & 0.455 & 0.612 & 0.480 & 0.449 & 0.611 & 0.483 \\ \hline
x101-64x4d-distilroberta-1c    & 0.448 & 0.584 & 0.476 & 0.439 & 0.580 & 0.469 \\ \hline
...
\begin{table}[H]
  \begin{center}
    \caption{Gradually increasing dataset size - 6 classes - In-set (\Cref{subsub:inset})}
...
\begin{table}[H]
  \begin{center}
    \caption{Gradually increasing dataset size - 6 classes - Near-set (\Cref{subsub:nearset})}
    \label{tab:dsizeallcnearset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l|l}
    \textbf{\% Train} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} & \textbf{$\sigma$} \\
    \hline
    25\% & 0.360 & 0.455 & 0.374 & 0.343 & 0.455 & 0.374 & 0.006 \\    \hline
    50\% & 0.381 & 0.449 & 0.412 & 0.377 & 0.449 & 0.409 & 0.005 \\    \hline
    75\% & 0.420 & 0.502 & \textbf{0.457} & 0.411 & 0.502 & \textbf{0.446} & 0.011 \\    \hline
    90\% & \textbf{0.434} & \textbf{0.528} & 0.453 & \textbf{0.425} & \textbf{0.531} & \textbf{0.446} & 0.009 \\    \hline
    100\% & 0.414 & 0.493 & 0.426 & 0.400 & 0.502 & 0.418 & 0.012 \\    \hline
...
\begin{table}[H]
  \begin{center}
    \caption{Gradually increasing dataset size - 6 classes - Out-set (\Cref{subsub:outset})}
    \label{tab:dsizeallcoutset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l|l}
    \textbf{\% Train} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} & \textbf{$\sigma$} \\
    \hline
    25\% & 0.296 & 0.393 & 0.314 & 0.286 & 0.391 & 0.312 & 0.009 \\    \hline
    50\% & 0.421 & 0.505 & 0.452 & 0.405 & 0.500 & 0.444 & 0.023 \\    \hline
    75\% & 0.437 & 0.515 & 0.471 & 0.424 & 0.515 & 0.458 & 0.008 \\    \hline
    90\% & \textbf{0.501} & \textbf{0.600} & 0.540 & 0.489 & \textbf{0.598} & \textbf{0.535} & 0.021 \\    \hline
    100\% & 0.499 & 0.590 & \textbf{0.543} & \textbf{0.490} & 0.590 & 0.533 & 0.016 \\    \hline
    \end{tabular}}
  \end{center}
\end{table}
...
\begin{table}[H]
  \begin{center}
    \caption{Gradually increasing dataset size - 1 class - In-set (\Cref{subsub:inset})}
    \label{tab:dsizeonecinset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l|l}
    \textbf{\% Train} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} & \textbf{$\sigma$} \\
    \hline
    25\% & 0.760 & 0.874 & 0.805 & 0.752 & 0.872 & 0.806 & 0.007 \\    \hline
    50\% & 0.848 & 0.919 & 0.886 & 0.843 & 0.922 & 0.886 & 0.004 \\    \hline
    75\% & \textbf{0.919} & \textbf{0.973} & \textbf{0.960} & \textbf{0.911} & \textbf{0.973} & \textbf{0.957} & 0.003 \\    \hline
    90\% & 0.904 & 0.962 & 0.935 & 0.895 & 0.961 & 0.934 & 0.004 \\    \hline
    100\% & 0.839 & 0.928 & 0.875 & 0.833 & 0.932 & 0.871 & 0.007 \\    \hline
...
\begin{table}[H]
  \begin{center}
    \caption{Gradually increasing dataset size - 1 class - Near-set (\Cref{subsub:nearset})}
    \label{tab:dsizeonecnearset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l|l}
    \textbf{\% Train} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} & \textbf{$\sigma$} \\
    \hline
    25\% & 0.477 & 0.605 & 0.518 & 0.472 & 0.607 & 0.514 & 0.005 \\    \hline
    50\% & 0.507 & 0.631 & 0.542 & 0.499 & 0.631 & 0.536 & 0.005 \\    \hline
    75\% & 0.530 & 0.652 & 0.568 & 0.520 & 0.650 & 0.560 & 0.008 \\    \hline
    90\% & 0.543 & 0.662 & 0.584 & 0.536 & 0.664 & \textbf{0.580} & 0.010 \\    \hline
    100\% & \textbf{0.546} & \textbf{0.668} & \textbf{0.585} & \textbf{0.539} & \textbf{0.669} & 0.577 & 0.006 \\    \hline
...
\begin{table}[H]
  \begin{center}
    \caption{Gradually increasing dataset size - 1 class - Out-set (\Cref{subsub:outset})}
    \label{tab:dsizeonecoutset}
    \resizebox{\columnwidth}{!}{
    \begin{tabular}{l|l|l|l|l|l|l|l}
    \textbf{\% Train} & \textbf{$mAP_m$} & \textbf{$mAP_m^{50}$} & \textbf{$mAP_m^{75}$} & \textbf{$mAP_{bb}$} & \textbf{$mAP_{bb}^{50}$} & \textbf{$mAP_{bb}^{75}$} & \textbf{$\sigma$} \\
    \hline
    25\% & 0.360 & 0.484 & 0.373 & 0.354 & 0.482 & 0.373 & 0.005 \\    \hline
    50\% & 0.417 & 0.534 & 0.440 & 0.406 & 0.531 & 0.432 & 0.007 \\    \hline
    75\% & 0.410 & 0.533 & 0.435 & 0.404 & 0.531 & 0.434 & 0.008 \\    \hline
    90\% & 0.435 & 0.545 & 0.462 & 0.427 & 0.542 & 0.455 & 0.012 \\    \hline
    100\% & \textbf{0.441} & \textbf{0.563} & \textbf{0.464} & \textbf{0.435} & \textbf{0.561} & \textbf{0.462} & 0.007 \\    \hline
    \end{tabular}}
  \end{center}
\end{table}
...
\begin{subfigure}{0.75\textwidth}
  ...
  \caption{1 class - r50 Mask R-CNN 75\%. See \Cref{tab:dsizeonecinset}.}
...
\begin{subfigure}{0.75\textwidth}
  ...
  \caption{1 class - r50 Mask R-CNN 100\%. See \Cref{tab:dsizeonecinset}.}
  \label{fig:vanilla_1c_1.0_inset}
\end{subfigure}}
\end{adjustbox}
  \caption{Normalized confusion matrices for the one class experiment on the In-domain test set found in \Cref{tab:dsizeonecinset}. Percentages are rounded down.}
...
\end{adjustbox}
  \caption{Normalized confusion matrix for r50 Mask R-CNN 75\%. See \Cref{tab:dsizeallcinset}. Percentages are rounded down.}
...
\end{adjustbox}
  \caption{Normalized confusion matrix for r50 Mask R-CNN 100\%. See \Cref{tab:dsizeallcinset}. Percentages are rounded down.}
...
\item The vanilla Mask R-CNN (resnet50-FPN) predictions visualized on a black background. The model was trained on 75\% of the training data, see the third row of \Cref{tab:dsizeallcinset}, corresponding to the best performing model in that table. The predicted class and confidence is depicted in the top left corner of each predicted bounding box.
\end{enumerate}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\clearpage
...
\includegraphics[width=\linewidth]{figures/labels-vanilla-0.75/JIefsDa.jpg}
  \caption{r50 Mask R-CNN 75\%. See \Cref{tab:dsizeallcinset}. }
...
\begin{enumerate}
\item Deciding the label width based on the column header width as seen in \Cref{subsub:labellistings} and \Cref{subsub:labelgame}. 
\item Deciding to label advertisements made to look like news articles as news articles in \Cref{subsub:labelads}.
\item Deciding that news units discussing the same topic with two bylines to be two instances of news units in \Cref{subsub:labelnewsdelim}.
\end{enumerate}

To illustrate this dilemma, imagine that the class News Unit found in \Cref{subsub:labelnewsunit} was divided into subclasses based on its textual contents. Such subclasses could include for example Political News Unit, Economical News Unit, or Historical News Unit. Perhaps a class division based on the semantic contents of the elements would have created more importance for the textual features, ultimately favoring models with access to textual features in the input and the ability to distinguish these subclasses from each other.

Some importance was encoded into the textual features. In \Cref{subsec:static}, we make a distinction based on the textual contents when labeling some textual elements as static. And of course, there is plenty of text elements found within the labels of other classes than News Unit to potentially aid in distinguishing between classes and borders of bounding boxes and segmentation masks. 
...
Because these elements should belong to one instance of a label as seen in \Cref{subsub:overlappinglabels}, they are labeled as "Advertisement".
...
As seen \autoref{tab:modeldimensions}, the sentence transformers output their vectors of size $768$ or $384$. This adds a significant number of channels to the input if inserted without alteration. The consequence is a large memory footprint. For instance, the final text embedding matrix size of a language model producing text embedding vectors of length $768$, is equal to that of $256$ RGB images. It should be stressed that an increased input size consequently increases training time and thus energy consumption, leading to a greater environmental impact overall.

\section{Impact of dataset size}

As seen in \Cref{tab:dsizeallcinset} and \Cref{tab:dsizeonecinset}, performance peaks at 75\% training size. Part of the explanation could be the curious distribution produced by random sampling as seen in \Cref{tab:partialdist}. Another explanation could be that the 75\% subset better represents the distribution of the test subset. Because of the size of the test subset (109 newspaper images), it is more than possible that the test subset does not necessary reflect the diversity of the full dataset from which it was sampled. See \Cref{fig:size_dist_annotations_in} for a notion of the distribution in regards to annotation area.

As we can see in \Cref{tab:dsizeallcnearset,tab:dsizeallcoutset,tab:dsizeonecnearset,tab:dsizeonecoutset}, performance in near- and out-of-domain test sets improves with increasing train dataset size. This supports the aforementioned theory that there are more diverse examples left out of the 75\% subset.
...
\textbf{Problem statement}. In \Cref{sec:problem} we formualted the following problem statement:

\textit{How well can neural networks, developed for object detection and segmentation of real-world objects, perform in the domain of news article segmentation?}

Given the results reported on the in-domain test subset (DN 2010 - 2020) which is of the same newspaper topology as the training subset, we demonstrate that even the unimodal implementation of Mask R-CNN is able to exploit the spatial features of the newspaper article elements such that it can successfully learn to reproduce the label strategy found in \Cref{subsec:labelingstrat} for this specific newspaper topology, even with the caveats of processing individual pages found in \Cref{subs:caveatsindividualpages}.
...
\textbf{Goals}. In \Cref{sec:goals} we formulated the research questions:
...
As for the second goal, given that the results for the out-of-domain test set (\Cref{tab:top5allcoutset,tab:top5onecoutset}) consistently features multimodal networks to a larger extent than the in- and near-domain test sets suggests that the representations learned by the multimodal networks are more generally applicable than the unimodal varaints when trained on a limited topology. We cannot support such a conclusion for more diverse training sets.

As for the third goal, we report increasing performance on near- and out-of-domain test sets (\Cref{tab:dsizeallcnearset,tab:dsizeallcoutset,tab:dsizeonecnearset,tab:dsizeonecoutset}) with increasing dataset size. 

\section{Limitations}

\textbf{Pairwise merged newspaper pages.} Per newspaper page instance segmentation has its drawbacks. Newspapers are meant to be read mostly overviewing two pages simultaniously. "Implied content", as described in \Cref{subs:caveatsindividualpages} could be avoided by merging pages pairwise before annotation.
...
\textbf{Normalization of text embeddings}. When preprocessing images for object detection and segmentation, the RGB channels are normalized and zero-centered as seen in \Cref{subs:preprocessing}. No such strategy was evaluated for this thesis.
...
\textbf{Normalization of text embeddings}. When preprocessing images for object detection and segmentation, the RGB channels are normalized and zero-centered as seen in \Cref{subs:preprocessing}. Could a similar method improve results when applied separately to the vector representations of the word embedding language models?

\textbf{Dimensionality reduction}. Given the used method of text embeddings as seen in \Cref{subs:textembeddingstrat}, Principle Component Analysis (PCA) or any other dimensionality reduction algorithm could be used to compress the vector representation. This could be useful to limit the memory footprint of the input, and potentially strengthen the impact of the added dimensions.
...
\textbf{Text granularity}. As mentioned in \Cref{subs:textembeddingstrat}, there are many ways to segment the blocks of text recognized by the OCR. Further work is needed to determine the most effective way to present this information as input to deep learning models.
